...
class Appsecurity
{
    ...
    function check_login($username, $password){
      ...
      // $where_field = array('username','password');
      // $field_data = array($username,$password_md5);

      $select = array('username','password_saltr');
      $where = array(
        'username' => $username,
        'password' => $password_md5
      );

      // $this->ci->dbm->select('username,password_salt');
      // $this->ci->dbm->set_table(db_prefix.'tb_m_user');      

      $result = $this->ci->dbm->get_where('tb_m_user', $select, $where);
